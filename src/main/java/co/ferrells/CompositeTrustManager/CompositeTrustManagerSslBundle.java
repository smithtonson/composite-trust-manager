...
/**
 * Factory that composes a new Spring Boot {@link SslBundle} whose trust side is backed by a
 ...
 *
 * <p>Key material (private keys and the key store) from the source bundle is preserved without
 * modification; only the trust manager and trust store are replaced.
 *
 * <p>This class is package-private and is not intended to be used directly outside of
...
final class CompositeTrustManagerSslBundle {
     ...
     *
     * <p>The JVM default trust store (loaded via
     ...
     * superset of both. The key store from the source bundle is passed through unchanged.
     *
     ...
     * {@code target}.
     *
     * <p>Alias conflicts between the two stores are resolved by {@link #uniqueAlias}. Entries
     ...
     *
     * <p>If {@code alias} is not already in use it is returned as-is. Otherwise, a numeric
